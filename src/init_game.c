...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/15 09:26:50 by dajimene          #+#    #+#             */
/*   Updated: 2023/11/15 09:26:51 by dajimene         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init(t_game_data *game)
{
	...
	game->window = mlx_new_window(game->mlx_ptr, (SIZE * game->n_col + SIZE),
			(SIZE * game->n_row + SIZE), "SO LONG ADVENTURE TIME!");
	if (!game->window)
		free_and_exit(game, 2);
	init_imgs(game);
	ft_draw_map(game);
